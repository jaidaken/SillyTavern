        ...
        for (const { fn } of sorted) {
            try {
                ...
                if (typeof result !== 'string') {
                    log.ext.warn(`[MessageFormatter] Hook at stage '${stage}' returned ${/** @type {unknown} */ (result) instanceof Promise ? 'a Promise (hook may be async)' : typeof result} instead of a string. The hook's return value has been ignored.`);
                } else {
                    mes = result;
                }
            } catch (e) {
                log.ext.error(`[MessageFormatter] Hook error at stage '${stage}':`, e);
            }
        }
        return mes;
    }
