...
async function fetchModelsByModality(endpoint, inputModality, outputModality, mapFn = null) {
    ...
    if (!response.ok) {
        log.net.warn('OpenRouter API request failed', response.statusText);
        return [];
    }
    ...
    if (!Array.isArray(data?.data)) {
        log.net.warn('OpenRouter API response was not an array');
        return [];
    }
...
router.post('/credits', async (req, res) => {
    try {
        ...
        if (!response.ok) {
            log.net.warn('OpenRouter credits request failed', response.statusText);
            return res.sendStatus(500);
        }
...
router.post('/image/generate', async (req, res) => {
    try {
        ...
        log.net.debug('OpenRouter image generation request', req.body);
        ...
        if (!response.ok) {
            log.net.warn('OpenRouter image generation failed', await response.text());
            return res.sendStatus(500);
        }
        ...
        if (!imageUrl) {
            log.net.warn('No image URL found in OpenRouter response', data);
            return res.sendStatus(500);
        }
        ...
        if (!mimeType || !base64Data) {
            log.net.warn('Invalid image data format', imageUrl);
            return res.sendStatus(500);
        }
