...
check("mobile: topbar 11 taps >=44px reachable", tb.get("count") == 11 and not has(mob, "topbar-count", "topbar-tap", "topbar-reachable", "topbar-scroll"))  # w3-grp: +d-groups
check("mobile: 11 panels open >=80% wide in view", len(panels) == 11 and all(p.get("open") for p in panels) and not has(mob, "panel-open", "panel-width", "panel-inviewport"), f"{len(panels)} panels")  # w3-grp: +groups
