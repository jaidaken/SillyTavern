...
check("mobile: topbar 10 taps >=44px reachable", tb.get("count") == 10 and not has(mob, "topbar-count", "topbar-tap", "topbar-reachable", "topbar-scroll"))
check("mobile: 10 panels open >=80% wide in view", len(panels) == 10 and all(p.get("open") for p in panels) and not has(mob, "panel-open", "panel-width", "panel-inviewport"), f"{len(panels)} panels")
