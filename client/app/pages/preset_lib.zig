...
///
/// `preset` is the live half with its name stamped in, which is what the classic client writes
/// (`structuredClone(power_user.instruct)` with name assigned, preset-manager.js:656-659), so a
/// preset saved here is a preset it can read.
pub fn saveBody(a: Allocator, kind: Kind, name: []const u8, live: templates.Templates) Allocator.Error![]u8 {
    var root = std.json.Value{ .object = .empty };
    try templates.mergeTemplates(a, &root.object, live);
    const power_user = root.object.get("power_user").?.object;

    var preset = try overlay(a, power_user.get(@tagName(kind)).?.object, .empty);
...
test "saveBody posts the three keys the route reads" {
    ...
    const body = try saveBody(a, .instruct, "My Template", liveChatml());
...
test "saveBody names the context directory when the context half is saved" {
    ...
    var live = liveChatml();
    live.context.story_string = "{{#if description}}{{description}}\n{{/if}}";
    const sent = try parseResponse(a, try saveBody(a, .context, "Lighthouse", live));
...
test "a saved preset picks back up as the same template" {
    ...
    const sent = try parseResponse(a, try saveBody(a, .instruct, "Roundtrip", liveChatml()));
    ...
    // Saved from a live set with instruct ON, so the saved file carries `enabled` and picking it
    // back turns it on even from a default (disabled) starting set.
    try testing.expect(back.instruct.enabled);
}
...
test "saveBody cleans up on every allocation failure" {
    try testing.checkAllAllocationFailures(testing.allocator, struct {
        fn run(alloc: Allocator, name: []const u8) !void {
            var arena = std.heap.ArenaAllocator.init(alloc);
            defer arena.deinit();
            _ = try saveBody(arena.allocator(), .context, name, liveChatml());
        }
    }.run, .{@as([]const u8, "My Template")});
}
