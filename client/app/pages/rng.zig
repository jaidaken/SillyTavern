...
/// seedrandom's default ARC4 generator, seeded from a number exactly as `seedrandom(finalSeed)`:
/// `flatten` stringifies the number and appends a NUL, `mixkey` folds it into the ARC4 key, the key
/// schedule runs, and RC4-drop[256] discards the first 256 outputs.
pub const Seedrandom = struct {
    ...
    pub fn init(numeric_seed: i64) Seedrandom {
        ...
        const total = decimal.len + 1; // flatten appends a trailing NUL code unit.
        ...
        while (jj < total) : (jj += 1) {
            ...
            smear = smear ^ (cur *% 19);
            const ch: u32 = if (jj < decimal.len) decimal[jj] else 0;
...
test "seedrandom numeric-seed first double matches stock arc4 goldens" {
    try testing.expectEqual(@as(f64, 0.038085370776470735), firstDouble(0));
    try testing.expectEqual(@as(f64, 0.1776348083296759), firstDouble(1));
    try testing.expectEqual(@as(f64, 0.0016341939679719736), firstDouble(42));
    try testing.expectEqual(@as(f64, 0.7561153199796736), firstDouble(123456789));
    try testing.expectEqual(@as(f64, 0.8454316376100453), firstDouble(9007199254740991));
}

test "pickIndex reproduces stock getPickReplaceMacro indices byte-exact" {
    try testing.expectEqual(@as(usize, 0), pickIndex("Chat_2024-01-15@10h30m", "You see {{pick::red::green::blue}} ahead.", 8, 3));
    try testing.expectEqual(@as(usize, 1), pickIndex("default", "{{pick: sword, shield, bow, staff}}", 0, 4));
    try testing.expectEqual(@as(usize, 0), pickIndex("branch-2", "a {{pick::x::y}} b {{pick::x::y}} c", 19, 2));
    try testing.expectEqual(@as(usize, 1), pickIndex("café ☕", "The 🎲 rolls {{pick::north,south,east,west}}", 12, 4));
