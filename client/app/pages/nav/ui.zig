...
/// The slide is slide_ms (150ms); unmount a hair later so it fully plays before the panel drops.
const panel_close_ms: u32 = 175;
...
const slide_ms: f64 = 150;

// The performance object is a lifetime singleton, so resolve its handle once and keep it: a slide
// calls this every frame, and re-fetching it each time is a wasted boundary crossing per frame.
var perf_obj: ?js.Object = null;
fn nowMs() f64 {
    const perf = perf_obj orelse blk: {
        const p = js.global.get(js.Object, "performance") catch return 0;
        perf_obj = p;
        break :blk p;
    };
...
// Quad ease-out: quick then settling, but a shorter tail than cubic so the slide does not crawl the
// final few pixels (those sub-pixel end frames read as stutter). The drawer feel without a bezier solver.
