...
### S4b PROBE RESULTS (RAN THE TOOLS, 2026-07-14; supersedes the assumptions below where they conflict)

1. ZIEX PLUGIN SILENTLY DROPS UTILITIES. `pkg/plugin-tailwindcss/src/builder.js:86` `extractCandidates` = ONE regex `[a-zA-Z0-9_\-\/:.!\[\]#%]+` with NO parens, `&`, `>`. measured against the official CLI (oxide) on one markup file: `text-red-500` OK, `gap-3` OK, `data-selected:bg-black` OK, but **`w-[calc(100%-2rem)]` DROPPED, `bg-(--accent)` DROPPED, `[&>p]:mt-2` DROPPED**. build stays green, class stays in the HTML, no CSS exists = the SAME silent-failure class as the dead handlers. `bg-(--token)` is v4's idiomatic design-token reference, so this is not an edge case.
   FIX (proven working): patch builder.js to use the REAL scanner, already in node_modules: `const {Scanner} = require("@tailwindcss/oxide"); new Scanner({sources: compiler.sources}).scan()`. verified against our live config: 2417 candidates, finds our one real class. ~6 lines. goes in `client/patches/` as patch 10 (we already maintain 9 ziex patches, so the mechanism exists). ADOPTION IS GATED ON THIS PATCH.
2. `config.sources` (the Zig-side `sources: []LazyPath`) does `readFileSync(path)` per entry: it expects FILES. hand it a DIRECTORY -> EISDIR -> swallowed by a bare `catch {}` -> silently scans nothing. do not pass dirs from build.zig. real scanning comes from the `@source` globs INSIDE the css, walked by the plugin's own hand-rolled glob (relative patterns only; an absolute-path glob silently matches nothing).
3. SCANNING `*.zig` IS DEAD WEIGHT: `.zx` yields 619 candidates, `.zig` yields 2174 (mostly ordinary identifiers), and the .zig scan adds ~3.0 KB of CSS no markup uses (this is where `.shrink`/`.grow`/`.text-wrap` in today's app.css come from). under the new design Zig emits data-attributes, not class names, so `@source` narrows to `*.zx` ONLY and the junk disappears.
4. CASCADE LAYERS, THE MIGRATION KILLER: our hand-written CSS is imported AFTER tailwind and is UNLAYERED, while every tailwind utility lives in `@layer utilities`. unlayered beats layered ALWAYS, regardless of specificity (verified: `.char-item` sits at brace-depth 0 outside every `@layer`). so today EVERY app-base.css rule beats EVERY utility. naive migration = add utilities to a component, see NO visual change, conclude the utility "did not work".
   FIX: wrap the legacy sheet in a layer declared BEFORE utilities (`@layer legacy { @import "./app-base.css"; }` with `@layer theme, base, components, legacy, utilities;`). then a utility immediately overrides its legacy rule, migration becomes incremental and safe (add utilities -> they take effect -> the dead legacy rule is deleted at leisure), and the end state is an empty legacy layer that gets removed. WITHOUT this every migration commit is atomic-or-broken.
   KEEP UNLAYERED (deliberately): the generated-prose rules, so no stray utility can ever override message-body styling.
